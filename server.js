...
const { db, getSetting, setSetting, stripHtml, reindexItem, removeFromIndex, hasFts } = require('./lib/db');
...
// Oude WordPress-uploadpaden blijven werken
app.use('/wp-content/uploads', express.static(path.join(__dirname, 'public', 'uploads'), { maxAge: '7d' }));

// Sessiegeheim persistent opslaan zodat logins een herstart overleven
const secretFile = path.join(__dirname, 'data', 'session-secret.txt');
let sessionSecret;
try { sessionSecret = fs.readFileSync(secretFile, 'utf-8').trim(); } catch (e) {}
if (!sessionSecret) {
...
app.use((req, res, next) => {
  ...
  res.locals.adminName = (req.session && req.session.displayName) || '';
  res.locals.settings = {
...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      ...
      const exists = fs.existsSync(path.join(__dirname, 'public', 'uploads', safe));
...
app.post('/admin/posts/save', requireAdmin, (req, res) => {
...
app.post('/admin/pages/save', requireAdmin, (req, res) => {
  const id = parseInt(req.body.id || '0', 10) || 0;
  const title = String(req.body.title || '').trim() || '(zonder titel)';
  const content = String(req.body.content || '');
...
app.get('/admin/media', requireAdmin, (req, res) => {
  ...
  let files = fs.readdirSync(dir)
    ...
    .map(f => ({ name: f, url: '/uploads/' + f, mtime: fs.statSync(path.join(dir, f)).mtimeMs, isImage: /\.(jpe?g|png|gif|webp|svg)$/i.test(f) }));
  if (q) files = files.filter(f => f.name.toLowerCase().includes(q));
  files.sort((a, b) => b.mtime - a.mtime);
  res.render('admin/media', { pageTitle: 'Media', files: files.slice(0, 200), total: files.length, q, active: 'media' });
});

app.post('/admin/media/upload', requireAdmin, upload.array('files', 20), (req, res) => {
  if (req.query.json || (req.headers.accept || '').includes('application/json')) {
    return res.json({ files: (req.files || []).map(f => ({ url: '/uploads/' + f.filename, name: f.filename })) });
  }
  res.redirect('/admin/media');
});

// TinyMCE afbeelding-upload endpoint
app.post('/admin/media/tinymce', requireAdmin, upload.single('file'), (req, res) => {
...
app.post('/admin/api/inline-save', requireAdmin, (req, res) => {
...
app.get(/.*/, (req, res, next) => {
  ...
  if (!clean || clean.startsWith('admin') || clean.startsWith('uploads') || clean.startsWith('assets')) return next();
